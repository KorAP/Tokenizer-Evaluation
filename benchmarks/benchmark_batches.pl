...
# save file in docker
open(X, '>', $temp_dir . 'effi-100x-utf8.txt') or die $!;
print X $data;
close(X);
...
my $offsets = `cat ${temp_dir}/effi-100x-utf8.txt | ./Datok/datok tokenize -t ./Datok/testdata/tokenizer.matok --no-tokens --no-sentences -p -`;
my @offsets = split / /, $offsets;

unlink './corpus/effi-100x-utf8.txt';
