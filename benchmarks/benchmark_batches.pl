...
# save file in docker
open(X, '>', './corpus/effi-100x-utf8.txt') or die $!;
print X $data;
close(X);
...
my $offsets = `cat ./corpus/effi-100x-utf8.txt | ./Datok/datok tokenize -t ./Datok/testdata/tokenizer.matok --no-tokens --no-sentences -p -`;
